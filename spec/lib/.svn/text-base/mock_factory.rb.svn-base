module MockFactory
  ...
  def mock_form_helper_examples
    @mechanize_agent = WWW::Mechanize.new  
    WWW::Mechanize.stub!(:new).and_return(@mechanize_agent)
    aps_page = mock_mechanize_page("http://www.alphapropertyservices.com/", File.open("#{mock_path}/alphapropertyservices.html", "r").read)
    @mechanize_agent.stub!(:get).and_return(aps_page)
  end
